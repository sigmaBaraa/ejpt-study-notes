...
Completed in INE: No
INE Section: Section 1 - Assessment Methodologies
Resource Type: Course Note
Study Status: Not Started
Tool/Topic: Enumeration
...
1. **Start Metasploit and import the XML file**
...
1. **Ports scan on the second target machine using a BASH SCRIPT**
...
1. **Ports scan on the second target machine using METASPLOIT MODULE**
...
1. **Find the default UDP ports used by smbd.**

```bash
nmap -sU --top-ports 25 <target_IP> # In this case ports are 137, 138
```

1. **What is the WorkGroup name of samba server?**

```bash
nmap -Pn -sV -O <target_IP> # workgroup: <group_name>
```

1. **Find the exact version of samba server by using appropriate Nmap script.**
...
1. **Find the exact version of samba server by using smb_version metasploit module.**
...
1. **What is the NetBIOS computer name of samba server? Use appropriate nmap scripts.**

```bash
nmap --script smb-os-discovery.nse -p <port_number> <target_IP> # port no. tipically 445
```

1. **Find the NetBIOS computer name of samba server using nmblookup**
...
1. **Using smbclient determine whether anonymous connection (null session) is allowed on the samba server or not.**
...
1. **Using rpcclient determine whether anonymous connection (null session) is allowed on the samba server or not.**
...
2. **Does user “admin” or “commander” exist on the server machine? Connect to SMTP service using netcat and check manually.**
...
1. **What commands can be used to check the supported commands/capabilities? Connect to SMTP service using telnet and check.**
...
1. **How many of the common usernames present in the dictionary exist on the server. Use smtp-user-enum tool for this task.**

```bash
smtp-user-enum -U /usr/share/commix/src/txt/usernames.txt -t <target_IP>
```

1. **How many common usernames present in the dictionary exist on the server. Use suitable metasploit module for this task.**
...
1. **Connect to SMTP service using telnet and send a fake mail to root user.**
...
1. **Send a fake mail to root user using sendemail command.**
